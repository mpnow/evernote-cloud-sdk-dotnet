...
namespace EvernoteSDK
{
	public class ENHTMLtoENMLConverter
	{
		...
		public string ENMLFromHTMLContent(string htmlContent)
		{
            ...
            ////StringWriterWithEncoding stringWriter = new StringWriterWithEncoding(resultString, Encoding.UTF8);
		...
		{
			using (var resource = this.GetType().Assembly.GetManifestResourceStream(this.GetType().Assembly.FullName.Split(',')[0] + "." + resourceName))
            {
